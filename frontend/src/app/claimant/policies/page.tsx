...
interface Policy { id: string; policy_name: string; file_url: string; }
...
function DownloadButton({ policyId, policyName, user }: { policyId: string; policyName: string; user: any }) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `${policyName}.pdf`; a.click();
...
function PreviewButton({ policyId, policyName, user }: { policyId: string; policyName: string; user: any }) {
  ...
  const handlePreview = async () => {
    ...
    try {
      const res = await apiFetchAuth(`/api/policies/${policyId}/download`, user);
      if (!res.ok) { const data = await res.json(); throw new Error(data.detail || "Preview failed"); }
      setBlobUrl(URL.createObjectURL(await res.blob()));
    } catch (e) { alert(e instanceof Error ? e.message : "Preview failed"); setOpen(false); }
    finally { setLoading(false); }
  };
  ...
  return (
    <>
      ...
      {open && <PDFPreviewModal policyName={policyName} blobUrl={blobUrl} loading={loading} onClose={() => setOpen(false)} />}
    </>
  );
}
...
export default function ClaimantPoliciesPage() {
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-[26px] font-bold tracking-tight" style={{ color: "#050508" }}>{t("policyPlansTitle")}</h1>
        <p className="text-[14px] mt-0.5" style={{ color: "rgba(5,5,8,0.45)" }}>{t("policyPlansSub")}</p>
      </div>
      ...
      {fetching ? (
        ...
        <div className="rounded-2xl border p-8 text-center" style={{ borderColor: "#fecaca", background: "#fef2f2" }}>
          <p className="text-sm font-medium" style={{ color: "#dc2626" }} dir="ltr">{error}</p>
          <button onClick={fetchPolicies} className="mt-3 text-sm underline" style={{ color: "#dc2626" }}>{t("tryAgain")}</button>
        </div>
      ) : policies.length === 0 ? (
        <div className="rounded-2xl border p-16 text-center" style={{ borderColor: "#e2e2ee", background: "#fff", boxShadow: "0 1px 3px rgba(5,5,8,0.05)" }}>
          ...
          <p className="font-semibold" style={{ color: "#050508" }}>{t("noPolicies")}</p>
          <p className="text-sm mt-1" style={{ color: "rgba(5,5,8,0.4)" }}>{t("checkBackLater")}</p>
        </div>
      ) : (
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: "1px solid #f0f0f5" }}>
                  {[t("policyNameCol"), t("actionCol")].map((h) => (
                    <th key={h} className="px-5 py-3.5 text-left text-[11px] font-semibold uppercase tracking-widest"
              ...
              <tbody>
                {policies.map((policy, i) => (
                  <motion.tr key={policy.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold" style={{ color: "#050508" }} dir="ltr">{policy.policy_name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-4">
