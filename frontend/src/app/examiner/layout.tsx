...
export default function ExaminerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!loading && (!profile || profile.role !== "examiner")) {
    ...
    <div className="min-h-screen flex flex-col lg:flex-row" style={{ background: "#fafafd" }}>
      ...
      >
        ...
        <div className="px-6 pt-7 pb-8 border-b" style={{ borderColor: "#e2e2ee" }}>
          <Link href="/examiner/claims" className="inline-flex items-center gap-2">
            <div
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                style={{
                  background: isActive ? "rgba(0,4,232,0.07)" : "transparent",
                  color: isActive ? "#0004E8" : "rgba(5,5,8,0.55)",
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header
          ...
          style={{ background: "#fff", borderColor: "#e2e2ee" }}
        >
          <Link href="/examiner/claims" className="font-bold text-[15px] tracking-tight" style={{ color: "#050508" }}>
            Watheeq AI
          </Link>
          <button
            onClick={signOut}
