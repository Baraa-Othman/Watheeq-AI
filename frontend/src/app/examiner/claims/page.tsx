...
export default function ExaminerClaimsPage() {
  ...
  const handlePick = async (claimId: string) => {
    ...
    try {
      ...
      router.push(`/examiner/claims/${claimId}`);
    } catch (err: unknown) {
                ...
                >
                  <div className="flex items-start justify-between gap-4 flex-wrap">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      ...
                      {/* Under review / approved / rejected → View button (only mine) */}
                      {!isSubmitted && isMyReview && (
                        <button
                          id={`view-${claim.claimId}`}
                          onClick={() => router.push(`/examiner/claims/${claim.claimId}`)}
